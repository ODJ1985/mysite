...
def test_upload_file(base_url, file_path, file_size_mb):
    ...
    try:
        with open(file_path, 'rb') as f:
            ...
            expected_status = 200 if file_size_mb <= 50 else 413
            
            if response.status_code == expected_status:
                print(f"✅ Upload test passed - Status: {response.status_code}")
                if response.status_code == 200:
                    print(f"✅ File uploaded successfully: {response.json().get('file_id', 'No ID')}")
                else:
                    print(f"✅ File correctly rejected (too large): {response.json().get('detail', 'No detail')}")
                return True
            else:
                print(f"❌ Upload test failed - Expected {expected_status}, got {response.status_code}")
                try:
...
def main():
    ...
    for size_mb, result in results:
        ...
        expected = "Should succeed" if size_mb <= 50 else "Should fail"
        print(f"{status} - {size_mb}MB file upload ({expected})")
